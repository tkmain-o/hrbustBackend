function randname() {
  ...
  let randomName = '';
  if (twoOrThree > 0.97) {
    const a = Math.floor(Math.pow(Math.random(), 3) * humanNameFirst.length + 1) - 1;
    const b = Math.floor(Math.pow(Math.random(), 1.1) * humanNameDoubleForThree.length + 1) - 1;
    randomName = humanNameFirst[a] + humanNameDoubleForThree[b];
  } else if (twoOrThree > 0.6) {
    const m = Math.floor(Math.pow(Math.random(), 3) * humanNameFirst.length + 1) - 1;
    const n = Math.floor(Math.pow(Math.random(), 1.5) * humanNameSecondForTwo.length + 1) - 1;
    randomName = humanNameFirst[m] + humanNameSecondForTwo[n];
  } else {
    const x = Math.floor(Math.pow(Math.random(), 3) * humanNameFirst.length + 1) - 1;
    const y = Math.floor(Math.pow(Math.random(), 1.5) * humanNameSecondForThree.length + 1) - 1;
    const z = Math.floor(Math.pow(Math.random(), 1.5) * humanNameThirdForThree.length + 1) - 1;
